...
async function flip (suggestion, value) {
    fetch('http://to/do-undo-suggestion', {
        method: 'POST',
        body: JSON.stringify({
            suggestion,
            value
...
async function deleteSuggestion (suggestion) {
    fetch('http://to/delete-suggestion', {
        method: 'POST',
        body: JSON.stringify({
            suggestion
        })
...
async function show () {
    const raw = await fetch('http://to/suggestions.csv?cachebust=' + (Math.random() * 100000).toFixed(0))
